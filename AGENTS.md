...
### 筛选条件必须服务端过滤规则（2026-07-07 新增）
- **禁止在客户端做过滤**，所有筛选条件必须通过 API 传给 CRM 做服务端过滤
- _build_api_filters_from_settings 中所有条件必须发送到 CRM API，不得改为客户端过滤
- 客户端侧过滤（_obj_query_filter_rows）仅作为展示层的辅助逻辑，不可替代 API 过滤
- 违反此规则 = 必须修复，不允许合并
