...
from core import *
from network import *
from common import frameless_input_text, frameless_message_box, install_autofilter_header, apply_autofilter_to_table, AutoFilterHeader, CheckBoxAutoFilterHeader
...
# -- custom_report/address_extractor.py --
"""
地址提取器
...
# -- custom_report/formula_engine.py --
"""
公式引擎
...
# -- custom_report/dashboard/__init__.py --
"""
...
# -- custom_report/dashboard/ai_assistant.py --
"""
AI 助手 — 多提供商自然语言交互
...
# -- custom_report/dashboard/bridge.py --
"""
...
# -- custom_report/dashboard/chart_config_panel.py --
"""
图表配置面板
...
# -- custom_report/dashboard/dashboard_designer.py --
"""
仪表盘设计器
...
# 导入 Claude Code 深色主题（从 custom_report 共享）
try:
    from custom_report import (
        _CLAUDE_TERMINAL_THEME, _claude_apply_palette,
        _claude_scrollbar_stylesheet, _claude_message_html, _claude_markdown_to_html,
    )
except ImportError:
    _CLAUDE_TERMINAL_THEME = {'bg_primary': '#0D1117', 'bg_secondary': '#161B22',
        'bg_input': '#1C2128', 'text_primary': '#E6EDF3', 'text_secondary': '#8B949E',
        'text_muted': '#484F58', 'text_user': '#58A6FF', 'text_system': '#8B949E',
        'text_error': '#F85149', 'text_success': '#3FB950', 'border_default': '#30363D',
        'border_subtle': '#21262D', 'border_focus': '#58A6FF', 'accent_blue': '#58A6FF',
        'accent_green': '#3FB950', 'accent_orange': '#D29922', 'accent_purple': '#BC8CFF',
        'bg_hover': '#21262D', 'bg_accent_subtle': '#1A2332', 'bg_code_block': '#1E1E2E',
        'code_header_bg': '#161B22', 'code_header_text': '#8B949E',
        'font_mono': "'Cascadia Code', 'JetBrains Mono', 'Consolas', 'Courier New', monospace",
        'font_mono_size': '13px', 'font_small_size': '11px',
        'scrollbar_bg': '#161B22', 'scrollbar_handle': '#30363D', 'scrollbar_hover': '#484F58',
        'prompt_char': '❯', 'assistant_dot': '●'}
    def _claude_apply_palette(w): pass
    def _claude_scrollbar_stylesheet(): return ''
    def _claude_message_html(role, content): return f'<div>{content}</div>'
    def _claude_markdown_to_html(text): return text
        ...
        try:
            ...
            if json_str:
                ...
                if isinstance(parsed, dict) and 'chart_type' in parsed:
                    ...
                    self._append_message("hint", f"Chart definition detected: '{parsed.get('title', 'Untitled')}'. Click the button below to add it.")
...
# -- custom_report/dashboard/dashboard_list_page.py --
"""
仪表盘列表页
...
# -- custom_report/dashboard/dashboard_view_page.py --
"""
仪表盘查看页
...
# -- custom_report/dashboard/data_source_panel.py --
"""
数据源选择面板
...
# -- custom_report/dashboard/excel_importer.py --
"""
Excel / CSV 导入器
...
# -- custom_report/dashboard/html_template.py --
"""
...
# -- custom_report/dashboard/models.py --
"""
BI 仪表盘数据模型
...
# -- custom_report/dashboard/pbix_extractor.py --
"""
PBIX 元数据提取器
...
# -- custom_report/dashboard/pyecharts_renderer.py --
"""
pyecharts 渲染引擎 — 完整实现
===============================
...
# -- custom_report/dashboard/repository.py --
"""
仪表盘 + Excel 数据集持久化层
...
# -- custom_report/views/__init__.py --


# -- custom_report/views/field_config_panel.py --
"""
字段配置面板
...
class FieldConfigPanel(QWidget):
    ...
    def set_columns(self, columns: list[dict]):
        ...
        for row, col in enumerate(columns):
            ...
            self._table.setItem(row, 2, display_item)
    ...
    def get_columns(self) -> list[dict]:
        ...
        for row in range(self._table.rowCount()):
            display_item = self._table.item(row, 2)
    ...
    def _add_date_part_field(self):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            frameless_message_box(self, "错误", f"添加时间成分失败:\n{str(e)}")
...
# -- custom_report/views/filter_bar.py --
"""
筛选栏
...
# -- custom_report/views/preview_table.py --
"""
数据预览面板
...
class PreviewTable(QWidget):
    ...
    refreshDataReady = pyqtSignal(object, int)  # BG thread data ready (rows, total)
    ...
    def refresh(self):
        ...
        t = threading.Thread(target=self._do_refresh_bg,
            args=(snap_report_id, snap_page, snap_page_size,
                  ...
                  snap_filters, snap_order_by), daemon=True)
        t.start()
...
# -- custom_report/views/report_detail_page.py --
"""
报表详情页（数据视图）
...
# [merged] from .canvas.join_canvas import JoinCanvas
# [merged] from .canvas.table_card import TableCard
...
# -- custom_report/views/report_list_page.py --
"""
报表列表页
...
class ReportListPage(QWidget):
    ...
    refreshDataReady = pyqtSignal(object, object, str)
    ...
    def _setup_ui(self):
        ...
        self._table.setColumnWidth(8, 25)
        self._table.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
    ...
    def refresh(self):
        """Refresh list and folder tree (BG thread)"""
        if self._bg_processing:
            return
        search = self._search_input.text().strip() or None
        ...
        import threading
        t = threading.Thread(target=self._do_refresh_bg,
            args=(snap_current_folder, search), daemon=True)
        t.start()
    ...
    def _populate(self):
        ...
        for row, rpt in enumerate(page_data):
            ...
            self._table.setItem(row, 7, _item(status))
    ...
    def _on_batch_refresh(self):
        import sys
        import sys
        print('[BATCH] _current_page_ids=' + repr(self._current_page_ids)[:200], file=sys.stderr, flush=True)
        print('[BATCH] _checked_ids=' + repr(self._checked_ids)[:200], file=sys.stderr, flush=True)
        checked = [rid for rid in self._current_page_ids if rid in self._checked_ids]
...
class SQLEditorDialog(QDialog):
    ...
    def __init__(self, parent, sql: str, report, db, repo):
        ...
        self.setWindowTitle("MySQL SQL 编辑")
        self.resize(900, 650)
...
class TableSelectorPanel(QWidget):
    ...
    refreshDataReady = pyqtSignal(list, bool)
    ...
    def refresh(self, include_mysql_tables: bool = False):
        ...
        if _btn:
            _btn.setText("刷新中...")
        if not self._db or not self._db.available:
            self._show_placeholder("MySQL -", "Please configure MySQL")
            self._restore_refresh_btn()
            self._bg_processing = False
            return
        import threading
        t = threading.Thread(target=self._do_refresh_bg,
            args=(include_mysql_tables,), daemon=True)
        t.start()
    ...
    def _do_refresh_bg(self, include_mysql_tables: bool):
        try:
            ...
            try:
                ...
                if _conn: _conn.ping(reconnect=True)
            ...
            try:
                ...
                if result:
                    for row in result:
                        ...
                        if tname: entries.append({"table_name": tname, "source_type": "crm"})
            except Exception: pass
            try:
                sr_tables = self._db.execute("SHOW TABLES LIKE 'sr_%'")
                if sr_tables:
                    existing = {e["table_name"] for e in entries}
                    for row in sr_tables:
                        tname = list(row.values())[0] if row else ""
                        if tname and tname.startswith("sr_") and tname not in existing:
            ...
            if self._excel_repo:
                ...
                for ds in self._excel_repo.list_all():
                    tbl = ds.mysql_table
                    if tbl and tbl not in existing and self._db.table_exists(tbl):
                        entries.append({"table_name": tbl, "source_type": "excel"})
            for known_table in ("部门员工",):
                if self._db.table_exists(known_table) and known_table not in {e["table_name"] for e in entries}:
    ...
    def _on_refresh_data_ready(self, entries, include_mysql_tables):
        ...
        self._bg_processing = False
        if not entries:
            self._show_placeholder("Query failed", "MySQL disconnected")
            self._restore_refresh_btn()
            return
        self._list_widget.clear()
        self._table_data.clear()
        ...
        if not self._table_data:
            self._show_placeholder("No tables", "Sync data to MySQL first")
        else:
            self._count_label.setText(f"Total {len(self._table_data)} tables")
        self._restore_refresh_btn()
...
# -- custom_report/views/canvas/__init__.py --


# -- custom_report/views/canvas/canvas_scene.py --
"""
画布场景管理器
...
# -- custom_report/views/canvas/join_canvas.py --
"""
拼表画布主控件
...
# -- custom_report/views/canvas/join_line.py --
"""
连线
...
# -- custom_report/views/canvas/table_card.py --
"""
对象卡片
...
class ReportManager:
    ...
    def initialize(self):
        ...
        # [merged] from . import setup_ui_logging
        setup_ui_logging()
    ...
    def _on_batch_refresh_reports(self, report_ids, progress_callback=None, done_callback=None):
        ...
        if not report_ids:
            ...
            return
        self._append_output('[batch] ' + str(len(report_ids)) + ' reports...' + chr(10))
...
# [merged] from .views.report_list_page import ReportListPage
            ...
            try:
                ...
                print('[BATCH] refresh OK, reports=' + str(len(self._list_page._all_reports)), file=__import__('sys').stderr, flush=True)
            except Exception as e:
                print('[BATCH] refresh FAILED: ' + str(e), file=__import__('sys').stderr, flush=True)
        return self._list_page

    def get_editor_page(self):
        """获取报表编辑器页"""
        if self._editor_page is None:
# [merged] from .views.report_editor_page import ReportEditorPage
...
# [merged] from .dashboard.repository import DashboardRepository, ExcelDatasetRepository
    ...
    def get_dashboard_list_page(self):
        """获取仪表盘列表页"""
# [merged] from .dashboard.dashboard_list_page import DashboardListPage
        page = DashboardListPage(self.dashboard_repo)
        return page

    def get_dashboard_view_page(self):
        """获取仪表盘查看页"""
# [merged] from .dashboard.dashboard_view_page import DashboardViewPage
# [merged] from .dashboard.bridge import DashboardBridge
    ...
    def get_dashboard_designer(self):
        """获取仪表盘设计器"""
# [merged] from .dashboard.dashboard_designer import DashboardDesigner
# [merged] from .dashboard.bridge import DashboardBridge
        bridge = DashboardBridge()
    ...
    def import_excel_dataset(self, file_path: str) -> object:
        """导入 Excel/CSV 文件为数据集"""
# [merged] from .dashboard.excel_importer import ExcelImporter
        importer = ExcelImporter(self._db)
        dataset = importer.import_file(file_path)
...
# [merged] from custom_report.constants import get_mysql_type_for_field, is_numeric_field_type, safe_numeric_value, get_mysql_type_for_format, safe_value_by_format
...
class BatchRefreshWorker(QObject):
    ...
    report_finished = pyqtSignal(str, bool, str)
    all_done = pyqtSignal()
    def __init__(self, mysql_config, crm_client, app_config, report_ids, append_fn=None):
        super().__init__(None)
        self._mysql_config = mysql_config; self._crm_client = crm_client
        self._app_config = app_config; self._report_ids = report_ids
        self._append = append_fn or (lambda x: None)
    ...
    def run(self):
        ...
        def _refresh_one(rid, idx):
            try:
                rpt = self._repo.get(rid)
                if not rpt: return (rid, False, 'not found', 0, '?')
                name = rpt.name
                ...
                if not wrk: return (rid, False, 'no db', 0, name)
                sw = {'sync': 40, 'join': 35, 'calc': 20, 'done': 5}
                def scb(phase, msg, pct):
                    if self._cancelled: return
                    w = sw.get(phase, 0); sp = int(pct * w / 100) if w else 0
                    with lock: c = self._completed
                    self.progress_updated.emit(c, self._total, sp,
                        '[' + str(idx+1) + '/' + str(self._total) + '] ' + name + ': ' + msg)
                t0 = time.time()
                result = wrk.full_refresh(rpt, progress_callback=scb)
                elapsed = int(time.time() - t0)
                ok = result.get('success', False); err = result.get('error', '') if not ok else ''
                return (rid, ok, err, elapsed, name)
            except Exception as e: return (rid, False, str(e), 0, '?')
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    rid, ok, msg, elapsed, name = result
                    with lock: self._completed += 1
                    self.progress_updated.emit(self._completed, self._total, 100,
                        '[' + str(self._completed) + '/' + str(self._total) + '] ' + name + ': ' + ('done' if ok else 'fail'))
                    self.report_finished.emit(name, ok, msg)
                    if ok: self._append('[batch] OK ' + name + ': ' + str(elapsed) + 's' + chr(10))
                    else: self._append('[batch] FAIL ' + name + ': ' + msg + chr(10))
                except: pass
        self._append('[batch] done, ' + str(self._total) + ' reports' + chr(10))
        self.all_done.emit()
...
class custom_report_pageMixin:
    ...
    def _on_new_report_edit(self, report_id):
        ...
        try:
            ...
            logging.exception(f"_on_new_report_edit ??: {e}")
            print(f"_on_new_report_edit ??: {e}", file=sys.stderr, flush=True)
